...
class Client {
  ...
  update(tableName, _key, operations) {
    const makeOp = op => {
      ...
      thresholdBuf.writeUInt32BE(op.updateInstruction.treshold, 0)
      const setValueBuf = Buffer.allocUnsafe(4)
      setValueBuf.writeUInt32BE(op.updateInstruction.setValue, 0)

      op.updateInstruction.treshold = thresholdBuf //TODO wrong spelling in proto file!
...
if (require.main === module) {
  ...
  co(function*() {
    ...
    yield client.createTable(tableName, ['key1', 'key2']).catch(() => {}) //Ignore error
    ...
    //     treshold: 10, //TODO wrong spelling in proto file!
    //     setValue: 0
    //   },
    //   value: 1,
    //   defaultValue: 0

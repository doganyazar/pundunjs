  ...
  _waitForMessageWithId(cId, timeout=5000) {
    return new Promise((resolve, reject) => {
      this.messageMap.set(cId, data => {
        ...
        if (pdu) {
          debug('Decoded message', JSON.stringify(pdu))
          resolve(pdu)
        } else {
          reject(new Error(`Failed to decode the received data
            for corr id ${cId}.`))
        }
      })
      setTimeout(() => {
...
if (require.main === module) {
  ...
  co(function*() {
    ...
    const key2 = [{'id': '0002'}, {'ts': Date.now()}]
    yield client.connect()
    const tabExists = yield client.listTables()
      .then(r => {
        return r.find(t => t === tableName) === tableName
      })
    if(tabExists){
      assert.strictEqual(yield client.deleteTable(tableName), 'ok')
    }
    assert.strictEqual(yield client.createTable(tableName, ['id', 'ts'],
      [{numOfShards: 1}]), 'ok')
    assert.strictEqual(yield client.addIndex(tableName, [
      {
        column: 'name'
      },
      {
    ...
    assert.strictEqual(yield client.write(tableName, key, value), 'ok')
    const value2 =
    [ {'text': 'Some irrelevant text here and there'},
      ...
      {'blank': null},
      {'double': 99.45} ]
    assert.strictEqual(yield client.write(tableName, key2, value2), 'ok')

    assert.deepStrictEqual(yield client.read(tableName, key), value)
    assert.deepStrictEqual(yield client.read(tableName, key2), value2)
    ...
    assert.deepStrictEqual(posting[0].key, key)
    ...
    assert.deepStrictEqual(posting2[0].key, key2)

    const expectedRangeRes =
      [ {key: key2, columns: value2},
        {key: key, columns: value}]
    const readRangeRes = yield client.readRange(tableName, key2, key, 10)
    assert.deepStrictEqual(readRangeRes, expectedRangeRes)
    const readRangeNRes = yield client.readRangeN(tableName, key2, 2)
    assert.deepStrictEqual(readRangeNRes, expectedRangeRes)

    const kcpIt = yield client.first(tableName)
    assert.deepStrictEqual(kcpIt.kcp, {key: key2, columns: value2})
    const nextRes = yield client.next(kcpIt.it)
    assert.deepStrictEqual(nextRes, {key: key, columns: value})
    const kcpIt2 = yield client.seek(tableName, key)
    assert.deepStrictEqual(kcpIt2.kcp, {key: key, columns: value})
    const prevRes = yield client.prev(kcpIt2.it)
    assert.deepStrictEqual(prevRes, {key: key2, columns: value2})
    const prevRes2 = yield client.prev(kcpIt2.it)
    const invalidIteratorError = {error: '{error,invalid}', type: 'system'}
    assert.deepStrictEqual(prevRes2, invalidIteratorError)
    const lastRes = yield client.last(tableName)
    assert.deepStrictEqual(lastRes.kcp, {key: key, columns: value})
    ...
    assert.strictEqual(yield client.removeIndex(tableName, ['name']), 'ok')
    yield client.tableInfo(tableName, ['index_on'])
    assert.strictEqual(yield client.delete(tableName, key), 'ok')
  })
}
